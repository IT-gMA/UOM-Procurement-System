...
function format_json_category(json_category, for_insert=false){
    return {
        'category_uid': for_insert ? 'new-brand-1' : json_category.prg_ggorderbrandid,
...
function render_body_content(){
    $.ajax({
        ...
        complete: function(response, status, xhr){
            ...
            response.brands.forEach(category => {
...
$(document).ready(function(){
    ...
    APPLY_CATEGORY_INFO_CHANGES_BTN.on('click', function(event){
        ...
        CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            ...
            if (formatted_category === undefined) return;
            const updated_data = {'brand_uid': formatted_category.category_uid,
                                    'new_brand_name': $(this).find('input[name=category-name-input-field]').val(),
                                    'new_active_status': $(this).find('input[name=category-active-state-switch]').prop('checked'),
                                };
            if (updated_data.new_brand_name === formatted_category.category_name && updated_data.new_active_status === formatted_category.is_active) return;
            _updated_categories.push(updated_data);
        });
        if (_updated_categories.length < 1) return _ready_elem_for_changes(false, true);
        $.ajax({
            type: 'POST',
            url: 'https://prod-24.australiasoutheast.logic.azure.com:443/workflows/07927c5bf2514a1d84365727cafbcb72/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=GLZFiQHh6Fa34lVjR7doSxmiYR2S4oAEIBbArh2UUuo',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                _updated_categories.forEach(updated_category => {
                    const formatted_category = CATEGORIES.filter(category => category.category_uid === updated_category.brand_uid)[0];
                    const corresponding_tr = CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row][data-categoryuid='${updated_category.brand_uid}']`);
                    if (formatted_category === undefined || corresponding_tr.length < 1) return;
                    
                    formatted_category.category_name = updated_category.new_brand_name;
                    formatted_category.trimmed_category_name = clean_white_space(updated_category.new_brand_name.trim().toLowerCase());
                ...
                alert(`Successfully updated ${_updated_categories.length} brand${_updated_categories.length > 1 ? 's' : ''}.`);
            },
            error: function(response, status, xhr){
    ...
    APPLY_INSERT_CATEGORY_BTN.on('click', function(event){
        ...
        INSERTED_CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${INSERTED_CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            new_category = {
                'brand_name': $(this).find('input[name=category-name-input-field]').val(),
        ...
        $.ajax({
            type: 'POST',
            url: 'https://prod-15.australiasoutheast.logic.azure.com:443/workflows/2fbdaaeb72a442f48be21a4b11f468a6/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=RNxW6vMAwEztxzYZzLG_ZjTpxiXP124KTKD9spVAEno',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                const formatted_category = format_json_category(response.new_brand);
                CATEGORIES.push(formatted_category);
            ...
            error: function(response, status, xhr){
                if (status === 'timeout') return alert('Request timed out');
                alert('Failed to add new brand at this time');
            }});
    });
});