...
$(document).ready(function(){
    ...
    APPLY_CATEGORY_INFO_CHANGES_BTN.on('click', function(event){
        ...
        CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            ...
            if (formatted_category === undefined) return;
            const updated_data = {'subcategory_uid': formatted_category.category_uid,
                                    'new_subcategory_name': $(this).find('input[name=category-name-input-field]').val(),
                                    'new_active_status': $(this).find('input[name=category-active-state-switch]').prop('checked'),
                                };
            if (updated_data.new_subcategory_name === formatted_category.category_name && updated_data.new_active_status === formatted_category.is_active) return;
            _updated_categories.push(updated_data);
        });
        if (_updated_categories.length < 1) return _ready_elem_for_changes(false, true);
        $.ajax({
            type: 'POST',
            url: 'https://prod-22.australiasoutheast.logic.azure.com:443/workflows/6ae2aa37c2d148ec9504fbbce295d53a/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=gB5D5eLCovnfOb6PF_hNNTB-JCLN8vW-io0GCOTSGb8',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                _updated_categories.forEach(updated_category => {
                    const formatted_category = CATEGORIES.filter(category => category.category_uid === updated_category.subcategory_uid)[0];
                    const corresponding_tr = CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row][data-categoryuid='${updated_category.subcategory_uid}']`);
                    if (formatted_category === undefined || corresponding_tr.length < 1) return;
                    
                    formatted_category.category_name = updated_category.new_subcategory_name;
                    formatted_category.trimmed_category_name = clean_white_space(updated_category.new_subcategory_name.trim().toLowerCase());
                ...
                alert(`Successfully updated ${_updated_categories.length} subcategor${_updated_categories.length > 1 ? 'ies' : 'y'}.`);
            },
            error: function(response, status, xhr){
    ...
    APPLY_INSERT_CATEGORY_BTN.on('click', function(event){
        ...
        if (duplicating_names.length > 0) {
            alert(`Duplicating Subcategory names found at:\n${duplicating_names.sort().map(dupl_name => `\t${dupl_name}`)}`);
            return _ready_elem_for_changes(false, true);
        }
        ...
        INSERTED_CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${INSERTED_CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            new_category = {
                'subcategory_name': $(this).find('input[name=category-name-input-field]').val(),
        ...
        $.ajax({
            type: 'POST',
            url: 'https://prod-30.australiasoutheast.logic.azure.com:443/workflows/588c60cbc45d47b1b08db0064a736750/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=5kQ0NIEUsygb5HWUiDtJ8FiOtKBopRuzHLeWG5IPRNE',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                const formatted_category = format_json_category(response.new_subcategory);
                CATEGORIES.push(formatted_category);
            ...
            error: function(response, status, xhr){
                if (status === 'timeout') return alert('Request timed out');
                alert('Failed to add new sub category at this time');
            }});
    });
});