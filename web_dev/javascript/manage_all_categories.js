...
const CATEGORY_CARD_PREVIEW_MODAL= $('#ProductCardPreviewModal');
...
function make_category_tr_markup(formatted_category, target_table){
    return `<tr name='${target_table.attr('name')}-row' class='data-row'
                ...
                data-hexcolour='${formatted_category.hex_colour}'
            ...
            >
            <td name='category-card-preview-btn'><div class='image-container'><img src='https://i.ibb.co/vdPXt16/uom-category-card-preview-btn-icon.webp'></div></td>
            <td><div class='image-container' name='category-info-btn'><img src='${formatted_category.thumbnail_img}'/></div></td>
            ...
            <td><input maxlength='7' class='color-picker-input-field' type='color' placeholder='${formatted_category.hex_colour}' name="category-colour-input-field" value='${formatted_category.hex_colour}'/></td>
...
$(document).ready(function(){
    ...
    // Category Preview Function
    $(document).on('click', 'td[name=category-card-preview-btn]', function(event){
        CATEGORY_CARD_PREVIEW_MODAL.find('.modal-dialog').empty();
        const parent_tr = $(this).closest('tr');
        const preview_category_content = !is_product_table_row_from_insertion($(this)) ? {...CATEGORIES.filter(category => category.category_uid === parent_tr.attr('data-categoryuid'))[0]} : {...INSERTED_UOM_CATEGORY};
        preview_category_content.category_name = !parent_tr.find('input[name=category-name-input-field]').val() || is_whitespace(parent_tr.find('input[name=category-name-input-field]').val()) ? '' : parent_tr.find('input[name=category-name-input-field]').val();
        preview_category_content.hex_colour = !parent_tr.find('input[name=category-colour-input-field]').val() || is_whitespace(parent_tr.find('input[name=category-colour-input-field]').val()) ? '#E9E9E9' : parent_tr.find('input[name=category-colour-input-field]').val();
        render_category_card_preview(preview_category_content, CATEGORY_CARD_PREVIEW_MODAL.find('.modal-dialog'));
        CATEGORY_CARD_PREVIEW_MODAL.modal('show');
    });
    ...
    $(document).on('click', 'div[name=category-info-btn]', function(){
        const parent_tr = $(this).closest('tr');
        const formatted_category = parent_tr.attr('data-categoryuid') === INSERTED_UOM_CATEGORY.category_uid ? INSERTED_UOM_CATEGORY : CATEGORIES.filter(category => category.category_uid === parent_tr.attr('data-categoryuid'))[0];
        
        if (formatted_category === undefined) return;
        UPLOAD_IMG_MODAL.attr('data-targetedproduct', parent_tr.attr('data-categoryuid'));
        const has_empty_img = formatted_category.img_content === undefined || formatted_category.img_content_mime_type === undefined;

        PRODUCT_IMG_CONTAINER.empty();
        if (!has_empty_img) render_img_preview(PRODUCT_IMG_CONTAINER, covert_product_b64_img_to_url(formatted_category.img_content_mime_type, formatted_category.img_content));
    ...
    $(document).on('change drop', '#UploadImgInput', function(){
        ...
        const formatted_category = UPLOAD_IMG_MODAL.attr('data-targetedproduct') === INSERTED_UOM_CATEGORY.category_uid ? INSERTED_UOM_CATEGORY : CATEGORIES.filter(category => category.category_uid === UPLOAD_IMG_MODAL.attr('data-targetedproduct'))[0];
        ...
        resize_img(files[0], MAX_IMG_SIZE_KB, function(resized_file){
            is_b64_str_longer_than_allowed(resized_file, function(exceeded){
                ...
                PRODUCT_IMG_CONTAINER.empty();
                formatted_category.img_file = resized_file;
    ...
    UPDATE_PRODUCT_IMG_BTN.on('click', async function(event){
        const is_inserted_product = UPLOAD_IMG_MODAL.attr('data-targetedproduct') === INSERTED_UOM_CATEGORY.category_uid;
        const formatted_category = is_inserted_product ? INSERTED_UOM_CATEGORY : CATEGORIES.filter(category => category.category_uid === UPLOAD_IMG_MODAL.attr('data-targetedproduct'))[0];
        if (formatted_category === undefined) return;
        ...
        function _modify_img_updates(corresponding_tr, b64_content){
            formatted_category.img_content = is_removed ? undefined : b64_content[1];
            formatted_category.img_content_mime_type = is_removed ? undefined : b64_content[0];
            formatted_category.thumbnail_img = covert_product_b64_img_to_url(formatted_category.img_content_mime_type, formatted_category.img_content);

            corresponding_tr.find('div[name=category-info-btn]').empty();
            corresponding_tr.find('div[name=category-info-btn]').append(`<img src='${formatted_category.thumbnail_img}'>`);

            if (is_removed){
                PRODUCT_IMG_CONTAINER.empty();
                PRODUCT_IMG_CONTAINER.closest('.card-container').toggle(false);
                formatted_category.img_file = undefined;
            }
        }
        ...
        try{
            _ready_img_update();
            const b64_content = is_removed ? ['', ''] : get_base64_contents(await convert_to_base64(formatted_category.img_file));
            if (is_inserted_product){
                _modify_img_updates(INSERTED_CATEGORY_INFO_TABLE.find('tbody').find('tr'), b64_content);
                return on_img_update_success();
            }

            $.ajax({
                type: 'POST',
                url: 'https://prod-19.australiasoutheast.logic.azure.com:443/workflows/263f384b52f84b809958d08daf5f7903/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=xIj_3dwOfXLpIZxX_PyFvo91rJdBI7yK8sKxq1xmDgw',
                contentType: 'application/json',
                accept: 'application/json;odata=verbose',
                timeout: AJAX_TIMEOUT_DURATION,
                data: JSON.stringify({
                    'category_uid': formatted_category.category_uid,
                ...
                success: function(response, status, xhr){
                    _modify_img_updates(CATEGORY_INFO_TABLE.find(`tr[data-categoryuid='${formatted_category.category_uid}']`), b64_content);
                    alert(`Successfully ${is_removed ? 'removed' : 'updated'} thumbnail image for ${formatted_category.category_name}`);
                },
                error: function(response, status, xhr){
    ...
    $(document).on('change input', 'input[name=category-colour-input-field]', function(event){
        if ($(this).val()) $(this).closest('tr').attr('data-hexcolour', $(this).val());
        is_product_table_row_from_insertion($(this)) ? disable_insert_new_product_btn() : disable_button(APPLY_CATEGORY_INFO_CHANGES_BTN, false);
    });
    ...
    APPLY_CATEGORY_INFO_CHANGES_BTN.on('click', function(event){
        ...
        CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            ...
            if (formatted_category === undefined) return;
            const updated_data = {'category_uid': formatted_category.category_uid,
                                    'new_category_name': $(this).find('input[name=category-name-input-field]').val(),
                                    'new_hex_colour': clean_white_space(get_hex_value($(this).attr('data-hexcolour')).trim()),
                                    'new_active_status': $(this).find('input[name=category-active-state-switch]').prop('checked'),
                                };
            if (updated_data.new_category_name === formatted_category.category_name && updated_data.new_hex_colour === clean_white_space(get_hex_value(formatted_category.hex_colour).trim())
                && updated_data.new_active_status === formatted_category.is_active) return;
            _updated_categories.push(updated_data);
        });
        if (_updated_categories.length < 1) return _ready_elem_for_changes(false, true);
        $.ajax({
            type: 'POST',
            url: 'https://prod-21.australiasoutheast.logic.azure.com:443/workflows/c41daeef26b4448aa18a9a570af7949c/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=FRg6URo5nRmo3z2w1KtGFSjHTK-krO-KhwtHue6Ze10',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                _updated_categories.forEach(updated_category => {
                    const formatted_category = CATEGORIES.filter(category => category.category_uid === updated_category.category_uid)[0];
                    const corresponding_tr = CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${CATEGORY_INFO_TABLE.attr('name')}-row][data-categoryuid='${updated_category.category_uid}']`);
                    if (formatted_category === undefined || corresponding_tr.length < 1) return;
                    
                    formatted_category.category_name = updated_category.new_category_name;
                    formatted_category.trimmed_category_name = clean_white_space(updated_category.new_category_name.trim().toLowerCase());
                    formatted_category.hex_colour = `#${updated_category.new_hex_colour}`;
                    ...
                    corresponding_tr.attr('data-hexcolour', `#${formatted_category.new_hex_colour}`);
                ...
                alert(`Successfully updated ${_updated_categories.length} categor${_updated_categories.length > 1 ? 'ies' : 'y'}.`);
            },
            error: function(response, status, xhr){
    ...
    APPLY_INSERT_CATEGORY_BTN.on('click', function(event){
        ...
        INSERTED_CATEGORY_INFO_TABLE.find('tbody').find(`tr[name=${INSERTED_CATEGORY_INFO_TABLE.attr('name')}-row]`).each(function(){
            new_category = {
                'category_name': $(this).find('input[name=category-name-input-field]').val(),
                'hex_colour': clean_white_space(get_hex_value($(this).attr('data-hexcolour')).trim()),
                'active_status': $(this).find('input[name=category-active-state-switch]').prop('checked'),
                'img_content': INSERTED_UOM_CATEGORY.img_content ?? '',
                'img_content_mime_type': INSERTED_UOM_CATEGORY.img_content_mime_type ?? '',
            };
        });
        if (new_category === undefined) return _ready_elem_for_changes(false, true);
        $.ajax({
            type: 'POST',
            url: 'https://prod-16.australiasoutheast.logic.azure.com:443/workflows/4c3d75ae2d8b4711b441875aba0a9dd6/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=IaiKEDN9pvAJ9cT40K0UQk2TnIRuxuqzFb3fq4K9k1M',
            contentType: 'application/json',
            accept: 'application/json;odata=verbose',
            timeout: AJAX_TIMEOUT_DURATION,
            ...
            complete: function(response, status, xhr){
                ...
                const formatted_category = format_json_category(response.new_category);
                CATEGORIES.push(formatted_category);
            ...
            error: function(response, status, xhr){
                if (status === 'timeout') return alert('Request timed out');
                alert('Failed to add new category at this time');
            }});
    });
});